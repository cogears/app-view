...
function onBodyWheel(e: WheelEvent) {
    if (!scrollbar2Body && body.value && scrollbarY.value) {
        if (props.big) {
...
function onScrollbarYScroll() {
    if (scrollbar2Body && scrollbarY.value && body.value) {
        if (props.big) {
        } else {
...
function onScrollbarXScroll() {
    if (scrollbar2Body && scrollbarX.value && scrollBody.value && head.value) {
...
<template>
    <div class="data-table" :class="{ full: full, border: border }">
        <div class="thead frame text2">
            <div class="fixed" :class="{ shadowX: fixShadowX }">
                <div class="columns headers">
                    ...
                    <DataTableHead v-for="head in fixedHeaders" :head="head" :columns="columns"
                        :columnWidths="columnWidths" :sort="sort" @sort="onSortByColumn">
            ...
            <div class="scroller" ref="head">
                <div class="columns headers">
                    <DataTableHead v-for="head in scrollHeaders" :head="head" :columns="columns"
                        :columnWidths="columnWidths" :sort="sort" @sort="onSortByColumn">
                        <template v-for="(_item, key, index) in $slots" :key="index" v-slot:[key]="data">
                            <slot :name="key" v-bind="data"></slot>
                        </template>
            ...
            <div class="suffixed" :class="{ shadowX2: suffixShadowX }" v-if="suffixedHeaders.length">
                <div class="columns headers">
                    <DataTableHead v-for="head in suffixedHeaders" :head="head" :columns="columns"
                        :columnWidths="columnWidths" :sort="sort" @sort="onSortByColumn">
                        <template v-for="(_item, key, index) in $slots" :key="index" v-slot:[key]="data">
                            <slot :name="key" v-bind="data"></slot>
                        </template>
        ...
        <div class="tbody">
            <div class="tbody-scroller" :class="{ lock: big }" ref="body" @scroll.stop="onBodyScroll"
                @wheel.passive="onBodyWheel">
                <div class="tbody-content" :style="bodyStyle">
                    <div class="fixed" :class="{ shadowX: fixShadowX }">
                        <div class="columns" v-for="(item, i) in temp.dataSource"
                            :class="columnClass(i + temp.pageOffsetIndex)"
                            @mouseover="onMouseOver(i + temp.pageOffsetIndex)"
                            @mouseout="onMouseOut(i + temp.pageOffsetIndex)"
                            @mousedown="onMouseDown($event, i + temp.pageOffsetIndex)"
                            @click="onClick(i + temp.pageOffsetIndex, $event)" @dblclick="onExecute0"
                            @contextmenu="onContextMenu0(i + temp.pageOffsetIndex, $event)">
                            <span class="td-serial" v-if="serial">{{ i + temp.pageOffsetIndex + 1 }}</span>
                            <DataTableItem v-for="c in fixedColumns" :class="tdClass(c)" :style="tdStyle(c)"
                                :item="item" :column="c">
                    ...
                    <div class="scroller" ref="scrollBody">
                        <div class="columns" v-for="(item, i) in temp.dataSource"
                            :class="columnClass(i + temp.pageOffsetIndex)"
                            @mouseover="onMouseOver(i + temp.pageOffsetIndex)"
                            @mouseout="onMouseOut(i + temp.pageOffsetIndex)"
                            ...
                            @click="onClick(i + temp.pageOffsetIndex, $event)" @dblclick="onExecute0"
                            @contextmenu="onContextMenu0(i + temp.pageOffsetIndex, $event)">
                            <DataTableItem v-for="c in scrollColumns" :class="tdClass(c)" :style="tdStyle(c)"
                                :item="item" :column="c">
                                <slot :name="'td-' + c.key" :item="item" :column="c" :index="i + temp.pageOffsetIndex">
                                </slot>
                            </DataTableItem>
                    ...
                    <div class="suffixed" :class="{ shadowX2: suffixShadowX }" v-if="suffixedColumns.length">
                        <div class="columns" v-for="(item, i) in temp.dataSource"
                            ...
                            @click="onClick(i + temp.pageOffsetIndex, $event)" @dblclick="onExecute0"
                            @contextmenu="onContextMenu0(i + temp.pageOffsetIndex, $event)">
                            <DataTableItem v-for="c in suffixedColumns" :class="tdClass(c)" :style="tdStyle(c)"
                                :item="item" :column="c">
                                <slot :name="'td-' + c.key" :item="item" :column="c" :index="i + temp.pageOffsetIndex">
                                </slot>
                            </DataTableItem>
            ...
            <div class="scrollbar-y" ref="scrollbarY" @scroll.stop="onScrollbarYScroll"
                @mousedown="onMouseDownScrollbar" @wheel.prevent.passive v-if="full">
                <div :style="{ height: temp.contentHeight + 'px' }"></div>
            </div>
            <div class="scrollbar-x" ref="scrollbarX" @scroll.stop="onScrollbarXScroll"
                @mousedown="onMouseDownScrollbar" @wheel.prevent.passive>
...
.tbody {
    ...
    .columns {
        ...
        &.on {
            background-color: var(--table-selectedBack);
        }

        &.hover {
            background-color: var(--table-hoverBack);
        }
    }
